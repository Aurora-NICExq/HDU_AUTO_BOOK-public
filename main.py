...
class SeatAutoBooker:
    ...
    def login(self):
        ...
        try:
            ...
            # 表单校验依赖失焦，登录按钮在表单完整前带 disabled 样式类
            ...
            # 认证页 service= 参数里就带图书馆主机名，须等真正跳回图书馆域名
    ...
    def has_existing_booking(self):
        ...
        try:
            ...
            items = resp.json()["content"]["defaultItems"]
            # 列表含历史记录，status 为 0 表示进行中的预约
            return any(str(item.get("status")) == "0" for item in items)
        except Exception as e:
            logging.exception(e)
