...
每天 **20:27**、**20:57**（上海时区）自动跑：

1. 若后天对应星期的 `启用` 为 `false`，立即结束，不登录
2. 用学校统一身份认证（CAS）登录图书馆
3. 提交一单：后天 7:00–22:00；`name` 为区域名时在该区域随机抽座，为 `自定义` 时用文件底部的座位号
4. 座位被占会换一个再试；接口返回已有预约则停止，不重复提交
...
| `name`  | `二楼西`/`二楼东`/`四楼`/`六楼`/`十二楼`/`自定义` 等 | 预定座位位置    | name: 二楼西               |
...
> 截图为旧版配置，字段名是 `type`；现在请按上表改 `name`。
...
### 完成后它将于每天晚上 20:27 与 20:57 自动运行，预约**后天**的座位。你也可以在 `Actions` 标签中手动运行（同样会先检查 `启用`）。
